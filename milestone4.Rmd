...
#id_data$`lower.95%..ci` <- round(id_data$`lower.95%..ci`, 2)
#id_data$`upper.95%..ci` <- round(id_data$`upper.95%..ci`,2)
...
groupdatafinal<-filter(id_data, County %in% c("ALAMEDA", "SANTA CLARA", "SAN MATEO", "SAN FRANCISCO", "MARIN", "CONTRA COSTA", "SOLANO"))%>%rename("Lower 95% CI"= "Lower.95..CI")%>%rename("Upper 95% CI"= "Upper.95..CI")
...
ggplot(plotdata, aes(x=Year, y=Mean_Rate))+geom_line(stat="identity")+facet_wrap(vars(Disease_Type))+labs(x="Year", y="Mean Rate", title = "Mean rates of Infectious diseases (including STDs) in Bay Area Counties from 2001-2018") + theme_classic()
```
...
ggplot(plotdata2, aes(x=Year, y=Mean_Rate))+geom_line(stat="identity")+facet_wrap(vars(Disease_Type), ncol = 2)+labs(x="Year", y="Mean Rate", title = "Mean rates of Infectious Diseases in Bay Area Counties from 2001-2018") + theme_classic()
```

```{r}
plotdata3<-plotdata%>% filter(Disease_Type == "STD (Bacterial)")

ggplot(plotdata3, aes(x=Year, y=Mean_Rate))+geom_line(stat="identity")+facet_wrap(vars(Disease_Type))+labs(x="Year", y="Mean Rate", title = "Mean rates of STDs (Bacterial) in Bay Area Counties from 2001-2018") +theme_classic()
