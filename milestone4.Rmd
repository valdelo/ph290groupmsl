...
```{r, echo=FALSE}
ggplot(hope, aes(x=Year, y=Overall_Rate))+facet_wrap(vars(Disease_Type), ncol = 2, scales = "free_y")+geom_line(aes(color = Disease_Type))+labs(x="Year", y="Overall Rate (per 100K)", title = "Figure 6: Overall Rates of Infectious diseases (including STDs)", subtitle =  "In Bay Area Counties from 2001-2018") + theme_minimal()
```
...
```{r, echo=FALSE}
ggplot(std_set, aes(x=Year, y=Overall_Rate))+geom_line(aes(color = Sex))+labs(x="Year", y="Overall Rate (per 100K)", title = "Figure 7: Overall rates per year of STDs (Bacterial)", subtitle="In Bay Area Counties from 2001-2018 seperated by Sex") +theme_minimal()
...
### Methods for Table 4:
The goal was to visual the overall rate of each STD (Chlamydia, Gonorrhea, Syphilis) within the Bay Area over the years 2014-2018, in a table.
...
newSTD1$"2018"<-round(newSTD1$`2018`,2)
  ...
  kable(newSTD1,
caption="Table 4: Rate (per 100,000) of STDs in the Bay Area from 2014-2018")%>% kable_styling(latex_options = "striped") 
```
